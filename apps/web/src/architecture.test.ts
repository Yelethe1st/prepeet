...
describe("boundaries", () => {
   ...
   */
  it("keeps the product out of the design system", () => {
    const crossings: string[] = [];

    for (const file of sourceFiles(join(root, "design-system"))) {
      for (const specifier of imports(file)) {
  ...
  /** Nothing in lib renders. Its README says so, and now something checks. */
  it("renders nothing in lib", () => {
    const rendering = sourceFiles(join(root, "lib")).filter((file) => file.endsWith(".tsx"));

    expect(rendering.map(named)).toEqual([]);
  });
  ...
  it("describes every area of the tree", () => {
    const undescribed = ["app", "design-system", "features", "lib"].filter((area) => {
      try {
