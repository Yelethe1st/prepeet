...
describe("themes", () => {
  it("defines every semantic colour in both themes", () => {
    const missingFromDark = colorTokens.filter((name) => !dark.has(name));
    const missingFromLight = colorTokens.filter((name) => !light.has(name));

    expect({ missingFromLight, missingFromDark }).toEqual({
      missingFromLight: [],
      missingFromDark: [],
    });
  });
