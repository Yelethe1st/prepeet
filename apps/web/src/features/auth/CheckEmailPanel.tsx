...
const described: Record<TokenEmailKind, { subject: string; validity: string }> = {
  password_reset: { subject: "Set a new Prepeet password", validity: "30 minutes, and one use only" },
  magic_link: { subject: "Your sign-in link", validity: "15 minutes, and one use only" },
  otp: { subject: "Your verification code", validity: "10 minutes, and one use only" },
  verify_email: { subject: "Verify your email address", validity: "30 minutes, and one use only" },
};
...
export function CheckEmailPanel({ kind, email, request, changeAddressHref }: CheckEmailPanelProps) {
  ...
  async function resend() {
    ...
    try {
      ...
      if (error instanceof ApiError && error.code === "RESEND_COOLING_DOWN") {
        ...
        // the one that holds.
        setCooldown(error.retryAfterSeconds > 0 ? error.retryAfterSeconds : assumedCooldownSeconds);
        return;
      }
      setFailure(error instanceof ApiError ? error.message : "That did not send. Try again.");
    }
  }

  return (
    <div>
      <p className="text-sm text-fg-2">We have sent a single-use link to the address below.</p>

      <dl className="mt-5 rounded-md border border-border bg-surface p-4 text-sm">
        <div className="flex justify-between gap-4">
          <dt className="text-fg-2">Sent to</dt>
          <dd className="font-medium">{email ? maskEmail(email) : "the address you gave"}</dd>
        </div>
      ...
      {resent ? <Banner tone="success">A new email is on its way. Only the newest one works.</Banner> : null}
      {failure ? <Banner tone="danger">{failure}</Banner> : null}

      <div className="mt-6">
        <p className="text-sm text-fg-2">
          Nothing yet? You can send it again once the cooldown finishes. We limit resends to stop
          mailbox flooding.
        </p>
        <div className="mt-3">
          <Button type="button" variant="secondary" disabled={cooldown > 0 || !email} onClick={() => void resend()}>
