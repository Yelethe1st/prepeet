...
describe("RequestEmailForm", () => {
  it("requests the flow's email and reports the address it used", async () => {
    ...
    const onSent = vi.fn();
    render(<RequestEmailForm kind="password_reset" request={request} onSent={onSent} />);
  ...
  it("treats a cooldown as sent, because the recent email is the one to check", async () => {
    const request = vi.fn().mockRejectedValue(
      new ApiError({ status: 429, code: "RESEND_COOLING_DOWN", message: "wait" }),
    );
    const onSent = vi.fn();
    render(<RequestEmailForm kind="magic_link" request={request} onSent={onSent} />);
  ...
  it("keeps a real failure on the form, with the reference support would ask for", async () => {
    const request = vi.fn().mockRejectedValue(
      new ApiError({ status: 500, code: "INTERNAL", message: "Something went wrong.", requestId: "req_9" }),
    );
    const onSent = vi.fn();
  ...
  it("refuses a non-address before any request is made", async () => {
    const request = vi.fn();
    render(<RequestEmailForm kind="password_reset" request={request} onSent={vi.fn()} />);
  ...
  it("states the flow's expiry, which the email will repeat", () => {
    render(<RequestEmailForm kind="password_reset" request={vi.fn()} onSent={vi.fn()} />);
    expect(screen.getByText(/30 minutes/)).toBeInTheDocument();
  });

  it("has no accessibility violations", async () => {
    const { container } = render(
      <RequestEmailForm kind="password_reset" request={vi.fn()} onSent={vi.fn()} />,
    );
