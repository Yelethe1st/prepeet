...
describe("TokenConsumer", () => {
  it("shows the checking state, then the done state", async () => {
    let release: () => void = () => {};
    const consume = vi.fn().mockReturnValue(new Promise<void>((resolve) => (release = resolve)));

    render(<TokenConsumer {...props} token="vrf_x" consume={consume} done={<p>All confirmed</p>} />);
  ...
  it("presents the token exactly once across re-renders", async () => {
    ...
    const { rerender } = render(
      <TokenConsumer {...props} token="vrf_x" consume={consume} done={<p>done</p>} />,
    );
    await screen.findByText("done");
    rerender(<TokenConsumer {...props} token="vrf_x" consume={consume} done={<p>done</p>} />);
  ...
  it.each([
    ...
    render(<TokenConsumer {...props} token="vrf_x" consume={consume} done={<p>done</p>} />);
  ...
  it("leads every dead state with what did not happen", async () => {
    ...
    expect(await screen.findByText(/nothing has changed on your account/i)).toBeInTheDocument();
  });

  it("treats an absent token as invalid without calling anything", () => {
    const consume = vi.fn();
    render(<TokenConsumer {...props} token="" consume={consume} done={<p>done</p>} />);
  ...
  it("offers the way forward from a superseded link without a resend button", async () => {
    ...
    expect(screen.queryByRole("link", { name: /send a new link/i })).not.toBeInTheDocument();
