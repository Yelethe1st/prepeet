...
| [0021](0021-goals-and-personal-requirements-live-in-progression.md) | Goals and personal requirements are Progression's rather than the candidate profile's: both are computed from the observation history and cannot be read without it, and placing them in Candidate would need either a boundary violation, a port exporting most of progression, or one invariant split across two contexts. Practice data stays candidate-owned with no tenant column at all | 2026-08-31 |
