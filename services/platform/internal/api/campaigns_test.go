...
func serveCampaigns(t *testing.T, stub *stubRecruiting) http.Handler {
	...
	handler, err := api.NewServer(api.ServerConfig{
		...
		Invitations: defaultStubInvitations(), ScreeningInvitations: defaultStubScreening(), CandidateAccommodations: defaultStubScreening(), ReInvitations: defaultStubInvitations(), Requirements: defaultStubRequirements(), RecruiterAccommodations: defaultStubInvitations(), Recruiting: stub, Environment: config.EnvironmentLocal,
	})
	if err != nil {
