...
func serveScreening(t *testing.T, stub *stubScreening) http.Handler {
	...
	handler, err := api.NewServer(api.ServerConfig{
		...
		ScreeningInvitations: stub, CandidateAccommodations: stub, ReInvitations: defaultStubInvitations(), Requirements: defaultStubRequirements(), RecruiterAccommodations: defaultStubInvitations(), Environment: config.EnvironmentLocal,
	})
	if err != nil {
